...
version: 1.5
changelog:
  - v1.5 (2026-05): 격리 sub-agent 병렬 페르소나 dispatch (parallel-persona-design.md), 등급별 Fact-Checker (fact-checker.md), 한국어 AI 작문 디텍션 (korean-ai-detection.md), Bull vs Bear 토론 라운드 (회사 분석), cross-talk 라운드 (페르소나 평가). 결정적 docx 변환 스크립트 (scripts/build_docx.py). 회귀 검증 하네스 골격 (evaluations/).
  - v1.3 (2026-05): 프론트매터 보강, references TOC, 회사 분석 검색 타입 강제(3+3+2+2), user-profile.md 분리
  - v1.2 (2026-05): 분석 보고서 docx 산출물 추가
  - v1.1 (2026-05): Step 4-0 정체성 게이트, Stage 0 Fact-Checker, 의사결정 매트릭스
  - v1.0: 최초 작성
---

# Career JD Matcher (v1.5)
...
| # | 변경 | 상세 참조 |
|---|------|----------|
| 1 | Fact-Checker를 격리 sub-agent + 등급별(tiered) 게이트로 재설계 | `references/fact-checker.md` |
| 2 | Stage 1·2 페르소나 9명을 병렬 sub-agent로 dispatch | `references/parallel-persona-design.md` |
| 3 | 1라운드 후 핵심 비판자 코멘트를 공유하는 cross-talk 라운드 (선택적) | `references/parallel-persona-design.md` |
| 4 | BLACK 페르소나에 한국어 AI 작문 디텍션 (11번째 항목) | `references/korean-ai-detection.md` |
| 5 | 회사 분석에 Bull vs Bear 토론 라운드 (Section 1.8.5 신규) | `references/company-analysis.md` |
| 6 | docx 생성을 결정적 Python 스크립트로 대체 | `scripts/build_docx.py` |
| 7 | 회귀 검증 하네스 골격 | `evaluations/README.md` |
...
- 회사 분석 검색을 `3 재무 + 3 시장·뉴스 + 2 평판 + 2 경쟁사·JD 부서` 쿼터 강제

## v1.2 / v1.1 변경 (유지)

- 분석 보고서 docx 자동 생성 (산출물 3개)
- 정체성 확인 게이트 (Step 4-0), 의사결정 매트릭스 (Section 6)

## 핵심 원칙

### 1. 거짓 사실 절대 금지 (최우선)

사용자 경력기술서·자소서에 없는 경험을 **어떤 이유로도** 추가하지 않는다. 변조 사례 목록과 허용되는 리라이팅 범위는 `references/resume-refinement.md` "절대 금지 사항" 섹션 참조. 판단이 애매하면 **쓰지 않는다**.

**v1.5 추가**: Fact-Checker가 격리 sub-agent에서 등급별(Hard/Soft/Ambiguous) 판정을 수행. Hard 클레임 Fail 시 즉시 원상 복구 + 사이클 중단. Ambiguous는 사용자 응답 필요.

### 2. BCG 컨설턴트 관점의 회사 분석

회사 분석은 단순 회사 소개가 아니다. **재무 수치(매출 추세, 영업이익률, 현금흐름) + 산업 구조(Porter's 5 Forces) + 시장 전망 + 학자·이론 인용**으로 구성된 분석 보고서. **타입 강제 쿼터**(3+3+2+2)로 10회 검색.

**v1.5 추가**: 분석 본문 작성 후 Bull vs Bear 1라운드 토론으로 단일 관점 편향 보정. Section 1.9 종합 진단의 입력. `references/company-analysis.md` Bull/Bear 섹션 참조.

### 3. 멀티 페르소나 9명 병렬 평가 (v1.5 업데이트)

- **Stage 0**: ⚫ FACT-CHECKER — 격리 sub-agent, 3등급 판정
- **Stage 1 (5-Color)**: BLACK·RED·SILVER·BLUE·GOLD — 보편적 완성도
- **Stage 2 (채용 라인업)**: 헤드헌터·현업 매니저·HR — 실전 통과 가능성

**v1.5 핵심 변경**:
- Stage 1·2 페르소나 8명을 격리 sub-agent에 **병렬 dispatch** (시간 단축 + 컨텍스트 격리)
- 1라운드 후 **cross-talk 라운드** 선택적 적용 (핵심 비판자 코멘트 공유 후 재채점)
- BLACK 페르소나에 항목 (11) **한국어 AI 작문 표지 검출** 추가

### 4. 사용자 페르소나 존중 (v1.3+)

자소서 톤·산업 선호도·거버넌스 민감도 등은 `references/user-profile.md` 한 파일에서만 로딩. 파일 부재 시 일반 모드. **Step 4-0에서 1회 명시적 확인.**
...
웹검색 **10회 타입 쿼터** (`3 재무 / 3 시장·뉴스 / 2 평판 / 2 경쟁사·JD 부서`). BCG 프레임워크(Porter 5 Forces / BCG Matrix / SWOT / Christensen) 중 적용 가능한 것 선택. **v1.5: 본문 1.1~1.8 완성 후 Bull vs Bear 토론 (Section 1.8.5) → 1.9 종합 진단의 입력.**
...
### Step 5: 멀티 페르소나 평가 및 리라이팅 (v1.5 병렬 + cross-talk)
...
**2) .docx 파일 3개** — **v1.5: scripts/build_docx.py로 결정적 생성**
...
매니페스트 스키마는 `scripts/build_docx.py` 상단 docstring 참조. v1.5 이전의 docx-js 즉석 생성은 fallback으로만 유지.
...
이런 경우엔 일반 응답으로 처리하거나 적절한 별도 스킬로 안내.
