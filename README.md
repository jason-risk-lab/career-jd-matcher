...
A Claude skill that handles the full mid-career job application package in one shot — company analysis, JD-resume matching, resume refinement, cover letter refinement, and multi-persona evaluation.

경력직 이직 지원 패키지 전 과정(회사 분석 → JD 매칭 평가 → 경력기술서 정교화 → 자기소개서 정교화 → 멀티 페르소나 평가/리라이팅)을 한 번의 호출로 처리하는 Claude 스킬입니다.
...
4. **자기소개서 정교화** — 회사 고유 정보 반영, 1,000자 이내 압축, 본인 직무 정체성 일관 노출.
5. **멀티 페르소나 평가 (8명, 최대 3회차)** — 5-Color Harness 5명(BLACK·RED·SILVER·BLUE·GOLD) + 채용 의사결정자 3명(헤드헌터·현업 매니저·HR)이 각각 10점 만점으로 평가하고, 9점 미만 항목을 리라이팅하는 사이클을 최대 3회 반복.

## 산출물

- 채팅 내 마크다운 보고서 (스코어 대시보드 + 변경 전후 비교표 + 페르소나 평가표)
- 정교화된 경력기술서 `.docx`
- 정교화된 자기소개서 `.docx`

## 핵심 원칙

- **거짓 사실 절대 금지** — 점수를 올리기 위해서도, 매칭률을 높이기 위해서도, 없는 경험/자격증/수치를 추가하지 않습니다. 갭은 갭으로 두고 학습 의지·확장성으로 다룹니다.
- **BCG 컨설턴트 톤** — 단순 회사 소개가 아니라 재무 수치·산업 구조·시장 전망에 기반한 분석.
- **사용자 페르소나 존중** — 메모리/기존 자료에 드러난 직무 정체성을 임의로 바꾸지 않습니다.
...
```bash
git clone https://github.com/<your-username>/career-jd-matcher.git
```
...
스킬이 자동으로 5단계를 순차 실행합니다. 웹검색 10회 + 페르소나 평가 사이클로 몇 분 소요됩니다.
...
└── references/
    ...
    └── persona-evaluation.md          # Step 5: 8 페르소나 평가 사이클
```
